...
with col_a:
    with st.container():
        ...
        experiment_lab = col_1.text_input("working group")
        experiment_user = col_2.text_input("user name")
        experiment_name = col_3.text_input("image name")
        experiment_tag = (
            col_4.text_input(
        ...
        col_1.write(
            f"**{experiment_lab}_{experiment_user}_{experiment_name}\:{experiment_tag}**"
        )
        col_2.write(
            f"**{experiment_lab}_{experiment_user}_{experiment_name}_{experiment_tag}.sif**"
        )
...
# script generation
default_partition = os.getenv("DEFAULT_PARTITION")
local_registry = os.getenv("LOCAL_REGISTRY")

text_pull = f"""echo 'pull image + convert to sif'
apptainer pull docker://{local_registry}/{experiment_lab}_{experiment_user}_{experiment_name}:{experiment_tag}
...
text_content_list.append(
    f"#SBATCH --job-name='{experiment_lab}_{experiment_user}_{experiment_name}_{experiment_tag}'"
)
